...
function renderCustomerDirectory(data, language = 'fr', query = '') {
  ...
  const rows = data.customers.map(row => {
    if (!row || typeof row.name !== 'string' || typeof row.email !== 'string' ||
        ...
        (row.id !== undefined && !UUID.test(row.id))) throw new TypeError('Invalid customer');
    const edit = row.id ? `<br><a href="/internal/customer-contact?lang=${language}&amp;id=${row.id}">${t.edit}</a>` : '';
    return `<tr><th scope="row">${escapeHtml(row.name)}${edit}</th><td>${escapeHtml(row.email)}</td><td>${escapeHtml(row.address || '—')}</td></tr>`;
  }).join('');
