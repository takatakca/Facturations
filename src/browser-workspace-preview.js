...
function page(language, workspaceId, title, inner) {
...
</style></head><body><main><header><div class="brand">GROUPE TAKATAK</div><nav aria-label="Navigation"><a href="/internal/recent-workspaces?lang=${language}">${t.back}</a><a href="${edit}">${t.edit}</a><a href="/internal/workspaces/${workspaceId}/preview?lang=${t.other}" lang="${t.other}">${t.language}</a></nav></header><h1>${title}</h1><span class="badge">${t.badge}</span>${inner}<p class="notice" role="note">${t.disclaimer}</p></main></body></html>`;
}

function renderWorkspacePreview({ preview, id, revision, language = 'fr' }) {
  ...
  return page(language, id, t.title, inner);
}
...
function attachBrowserWorkspacePreview(server, { origin, workspaceStore }) {
  ...
  server.on('request', async (request, response) => {
    ...
    try {
      ...
      return send(response, 200, 'text/html; charset=utf-8',
        renderWorkspacePreview({ preview, id: match[1], revision: row.revision, language }));
    } catch (error) {
