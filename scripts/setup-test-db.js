...
async function main() {
  ...
  try {
    for (const migration of [
      ...
      '012_provider_issuance_execution.sql',
    ]) {
      const sql = fs.readFileSync(path.join(__dirname, '..', 'db', migration), 'utf8');
      await pool.query(sql);
    }
    console.info('Isolated test schema initialized with draft protection, staff invitations, login limits, MFA, revisioned workspaces, immutable submissions, customer contact history, issuance authorization gate and provider execution state');
  } finally {
    await pool.end();
  }
}
