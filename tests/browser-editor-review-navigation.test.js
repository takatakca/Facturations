...
const saved = revision => ({ id: ID, revision, content: { currency: 'CAD',
  customer: { name: 'Fictional', email: 'fictional@example.test' },
  notes: 'Synthetic only', lines: [], taxes: [] }, status: 'WORK_IN_PROGRESS',
...
function harness({ existing = false, loadDeferred = false } = {}) {
  ...
  for (let n = 1; n <= 5; n++) for (const field of ['description', 'quantity', 'price', 'discount', 'taxable']) ids.push(`line-${n}-${field}`);
  for (let n = 1; n <= 3; n++) for (const field of ['code', 'label', 'rate']) ids.push(`tax-${n}-${field}`);
  const listeners = new Map();
  const elements = Object.fromEntries(ids.map(id => [id, { value: '', checked: false,
    disabled: true, hidden: true, href: '', textContent: '', dataset: {},
    addEventListener(name, callback) { listeners.set(id + ':' + name, callback); } }]));
  const calls = [], pendingGets = [];
  ...
  const fetchMock = async (path, options = {}) => {
    ...
    if (path !== '/internal/workspaces' && path !== '/internal/workspaces/' + ID) throw Error('Unexpected route');
    const body = JSON.parse(options.body);
    if (path === '/internal/workspaces') {
      assert.match(body.creationKey, /^[a-f0-9]{32}$/);
      server.current = { ...saved(1), content: body.content };
    } else {
      assert.equal(body.expectedRevision, server.current.revision);
      server.current = { ...saved(server.current.revision + 1), content: body.content };
    }
    return { ok: true, json: async () => server.current };
  };
  vm.runInNewContext(readFileSync(join(__dirname, '../src/workspace-editor-client.js'), 'utf8'), {
    ...
    fetch: fetchMock, confirm: () => true, // Explicit consent to discard synthetic input on retry.
  }, { timeout: 1000 });
...
test('FR/EN editor has hidden preview, disabled fieldset and no prefilled tax assumptions', () => {
...
test('new draft unlocks after CSRF; preview only after save and disappears on unsaved edits or 409', async () => {
  ...
  h.edit('customer', 'Fictional client'); await h.submit();
  ...
  assert.equal(h.elements.preview.hidden, true);
  h.server.failWrite = true; await h.submit();
  assert.equal(h.elements.preview.hidden, true);
...
test('initial GET failure keeps inputs locked, prevents null-revision PUT, permits explicit retry', async () => {
  const h = harness({ existing: true, loadDeferred: true }); await h.settle();
  ...
  assert.equal(h.elements.preview.hidden, true);
  h.pendingGets.shift()({ ok: false, status: 503 }); await h.settle();
  assert.equal(h.elements['editing-fields'].disabled, true);
  assert.equal(h.elements.reload.disabled, false);
  h.edit('customer', 'Programmatic edit while disabled'); await h.submit();
  assert.equal(h.calls.filter(call => call.options.method === 'PUT').length, 0);
  const retry = h.listeners.get('reload:click')();
  assert.equal(h.pendingGets.length, 1);
  h.pendingGets.shift()({ ok: true, json: async () => saved(1) }); await retry;
  assert.equal(h.elements['editing-fields'].disabled, false);
  assert.equal(h.elements.preview.hidden, false);
...
test('reload hides preview while GET is pending and shows it only after confirmed revision', async () => {
  const h = harness({ existing: true, loadDeferred: true }); await h.settle();
  h.pendingGets.shift()({ ok: true, json: async () => saved(1) }); await h.settle();
  assert.equal(h.elements.preview.hidden, false);
  const retry = h.listeners.get('reload:click')();
  assert.equal(h.elements.preview.hidden, true);
  assert.equal(h.elements['editing-fields'].disabled, true);
  h.pendingGets.shift()({ ok: true, json: async () => saved(2) }); await retry;
  assert.equal(h.elements['editing-fields'].disabled, false);
  assert.equal(h.elements.preview.hidden, false);
