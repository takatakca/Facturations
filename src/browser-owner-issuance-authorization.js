...
<section class="summary"><div><strong>${t.recipient}</strong><p>${escapeHtml(p.customer.email)}</p></div><div><strong>${t.total}</strong><p>${escapeHtml(money(p.totalCents, lang))} CAD</p></div></section>
