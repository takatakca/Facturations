...
Le moteur fournisseur interne enregistre maintenant des tentatives `PREPARED → IN_FLIGHT → résultat` avec journal append-only. Un résultat `AMBIGUOUS` bloque toute nouvelle tentative jusqu’à une réconciliation explicite `FOUND` ou `NOT_FOUND`; seul `RECONCILED_NOT_FOUND` ou `FAILED_RETRYABLE` permet de préparer un nouveau retry. `CONFIRMED` exige un identifiant et un numéro fournisseur vérifiés. Ce moteur ne contient encore aucun appel réseau Wave.
...
2. Faire examiner et appliquer sur cette **seule base dédiée** les migrations `db/001` à `db/012`, dans l'ordre, avec plan de sauvegarde chiffrée et preuve de restauration avant données réelles. Les migrations 010 et 011 sont requises respectivement pour l'édition auditée des fiches clients et l'autorisation d'émission en attente du fournisseur.
