...
function fixtures() {
  ...
  return [
    ...
    { path: PATHS[2], status: 401, tls, headers: { ...security }, body: 'Unauthorized' },
    { path: PATHS[3], status: 401, tls, headers: { ...security }, body: '{"error":"UNAUTHORIZED"}' },
  ];
}
...
test('manual target is the exact isolated HTTPS origin and request paths are fixed', async () => {
  ...
  assert.deepEqual(PATHS, ['/health', '/internal/login?lang=fr',
...
test('fails closed on TLS problems, redirects, unprotected routes and missing security headers', () => {
  const cases = [
    ...
    [1, item => { item.body = '<html lang="fr">Not the MFA login form</html>'; }],
    [1, item => { delete item.headers['content-security-policy']; }],
    [2, item => { item.status = 200; }],
    [3, item => { item.headers['access-control-allow-origin'] = '*'; }],
    [3, item => { item.headers['x-powered-by'] = 'Express'; }],
  ];
