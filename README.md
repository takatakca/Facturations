...
Les cookies navigateur n'autorisent aucune écriture `/api/*` ni action Wave. Le pipeline fournisseur empilé prépare un mapping immuable, un état d'exécution anti-doublon, un transport GraphQL verrouillé et une réconciliation lecture seule des résultats ambigus. Ces transports restent **désactivés par défaut, non branchés à `app.js` et testés uniquement avec `fetch` simulé** : aucun appel Wave réel n'est effectué par la CI. Après une exécution fournisseur confirmée, `src/issued-invoice-registry.js` peut matérialiser un registre local append-only `ISSUED_CONFIRMED`, tout en gardant `deliveryState=NOT_AUTHORIZED` et le brouillon source inchangé. Voir [`docs/provider-execution-state.md`](docs/provider-execution-state.md), [`docs/wave-mapping-snapshot.md`](docs/wave-mapping-snapshot.md) et [`docs/issued-invoice-registry.md`](docs/issued-invoice-registry.md). La configuration d'une origine HTTPS dans Node ne prouve pas à elle seule la présence d'un certificat ni d'un proxy sécurisé.

**Non livrés ou non homologués :** première exécution Wave réelle autorisée, PDF officiel immuable et stockage, envoi client, paiements/remboursements, portail client, assistant IA/vocal, récupération MFA, restauration sauvegarde, revue fiscale/comptable et homologation TLS publique. Le registre local d'émission ne prouve pas à lui seul qu'une facture réelle a été créée sur un compte Wave de production. Le parcours Chrome isolé couvre maintenant MFA → autosauvegarde → conversion immuable → approbation interne → impression non émise; il ne prouve ni le staging public ni le nouveau formulaire client dans Chrome.
...
`package-lock.json` est présent. Pour les tests PostgreSQL, `FACTURATIONS_TEST_DATABASE_URL` doit pointer exclusivement vers `localhost` ou `127.0.0.1` et la base `/facturations_test`; ne jamais définir `FACTURATIONS_DATABASE_URL` dans le processus de test. Vérifier la cible avant `node scripts/setup-test-db.js`. Ce script applique uniquement dans cette base jetable les migrations **001 à 016**, dont 008 (espaces révisables), 009 (conversion immuable), 010 (révision/journal des fiches clients), 011 (autorisation d'émission), 012 (état persistant des tentatives fournisseur), 013 (snapshot immuable des mappings Wave), 014 (preuve de CREATE fournisseur), 015 (journal pré-réseau append-only) et 016 (registre local immuable des factures confirmées). La CI exécute `npm ci`, audit production, `npm run check`, `npm test` sous Node 20/22 et des parcours Chrome séparés avec données fictives.
...
2. Faire examiner et appliquer sur cette **seule base dédiée** les migrations `db/001` à `db/016`, dans l'ordre, avec plan de sauvegarde chiffrée et preuve de restauration avant données réelles. Les migrations 010 à 016 couvrent les contacts audités, l'autorisation d'émission, l'état fournisseur, le snapshot des mappings Wave, les preuves anti-doublon et le registre local des factures confirmées.
